...
class Mapper(object):
    ...
    def map(self, f, v):
        ...
        *v* iterable
        ...
        self.rad = self.data[0]
        self.wt = self.data[1]
        self.tau = self.data[2]
        self.addargs = self.data[3]
        
        self.zipped = [(i,j) for i in self.rad for j in self.wt]
        
        if len(self.workers)>0:
            chunksize, extra = divmod(len(self.zipped), len(self.workers)*4)
#            chunksize, extra = divmod(len(self.rad), len(self.workers)*4)
            if extra:
                chunksize += 1
        else:
        ...
        argindex = 0
        self.outlist = [None]*len(self.rad) 
        
        iter_element = iter(self.zipped)
#        iter_element = iter(self.rad)
        exit_loop = False
        chunk = []
        while not exit_loop:
            seq = []
...
#                    args = iter_element.next()
                ...
                job = Map_job(self.func, argindex, rval, wval, self.tau, self.addargs)
                argindex += 1
                seq.append(job)
