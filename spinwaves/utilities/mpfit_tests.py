...
def test_linfit(bounded = False, fixed = False):
    ...
    if fixed:
        parinfo[1]['tied'] = '-0.551705 * p[0]'

    if bounded:
        parinfo[0]['limited'] = [1.,1.]
        parinfo[0]['limits'] = [-4.,4.]
    ...
    if fixed:
        ...
        assert N.allclose(m.perror,N.array([ 0.00512251,  0.],dtype='float64'))
        assert N.allclose(N.array([chisq],dtype='float64'),N.array([33.7644910396],dtype='float64'))
        assert m.dof==9
        return

    assert N.allclose(m.params,N.array([ 3.20996572, -1.7709542 ],dtype='float64'))
    assert N.allclose(m.perror,N.array([ 0.00634576,  0.00541073],dtype='float64'))
    assert N.allclose(N.array([chisq],dtype='float64'),N.array([33.7644910396],dtype='float64'))
    assert m.dof==8
    return

def test_linfit_bounded():
    test_linfit(bounded=True)

def test_linfit_fixed():
    test_linfit(fixed=True)

def test_linfit_bounded_fixed():
    test_linfit(bounded = True, fixed=True)
